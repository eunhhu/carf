...
import type { HookInfo, HookConfig } from "~/lib/types";

function HooksTab() {
  const [typeFilter, setTypeFilter] = createSignal<HookInfo["type"] | "all">("all");
  ...
  async function handleImport() {
    ...
    try {
      ...
      const parsed = JSON.parse(text) as { hooks: HookConfig[] };
  ...
  async function handleToggle(hookId: string, currentActive: boolean) {
    const session = activeSession();
    if (!session) return;
    await toggleHook(session.id, hookId, !currentActive).catch((e) =>
  ...
  async function handleDelete(hookId: string) {
    const session = activeSession();
    if (!session) return;
    await deleteHook(session.id, hookId).catch((e) =>
