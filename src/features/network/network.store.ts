...
async function startCapture(sessionId: string): Promise<void> {
  try {
    await invoke("rpc_call", { sessionId, method: "startNetworkCapture", params: {} });
...
async function stopCapture(sessionId: string): Promise<void> {
  try {
    await invoke("rpc_call", { sessionId, method: "stopNetworkCapture", params: {} });
...
function setupNetworkListener(sessionId: string): () => void {
  const cleanup = listen<NetworkRequest>(`carf://network/request`, (request) => {
    if (!request) return;
    setState("requests", (prev) => [...prev, request]);
  });
